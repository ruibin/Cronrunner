...
class CDdFetchUrl
{
	 ...
	 * @param bool need_proxy 代理地址：端口
	 * @param string proxy_addr
	 * @param string proxy_type  HTTP|SOCK$|SOCKE5
	 * @param bool proxy_need_auth 
	 * @param string proxy_auth_type  BASIC or NTLM
	 * @param string proxy_auth_usr
	 * @param string proxy_auth_psw
	 */
    function __construct($need_proxy = false, $uniq_key = null, $proxy_type = null, $proxy_addr = null,
                         $proxy_need_auth = null, $proxy_auth_type = null, 
                         $proxy_auth_usr = null, $proxy_auth_psw = null)
    {
    	if (isset($uniq_key)) {
    		$this->uniq_key = $uniq_key;
    	}
        $this->impl_ = new FetchUrl($this->uniq_key, $need_proxy, $proxy_type, $proxy_addr,  
                                    $proxy_need_auth, $proxy_auth_type, 
                                    $proxy_auth_usr, $proxy_auth_psw);
        $this->method_ = "get";
        $this->cookies_ = array();
        $this->opt_ = array();
        $this->headers_ = array();
     ...
     *  - sendtimeout int,单位为s,通过修改HTTP Header来设置
     *  - readtimeout int,单位为s,通过修改HTTP Header来设置
     *  - totaltimeout int,单位为s,通过修改HTTP Header来设置总的请求+响应时间
     *  - followlocation bool 是否跟踪跳转的http header信息
     * @param string $name 
     * @param string $value 
     * @return void 
     * @author Elmer Zhang
     * @edited by hcb0825@126.com
     * @ignore
     */
    ...
    public function fetch( $url, $opt = NULL, $callback=NULL )
    {
        ...
        $fetch_res = $this->impl_->fetch($url, $opt, $this->headers_, $callback);
        $this->debug_msg = $this->impl_->debug_msg;
        $this->headers_ = $this->impl_->headerContent();
        return $fetch_res;
    }
    ...
    public function getAllDebugMessage() {
    ...
    private $uniq_key = "hcb0825@126.com";
    private $impl_;
    private $opt_;
    private $headers_;
    private $debug_msg;
...
/**
 * FetchUrl , the sub class of DdFetchurl
 *
 *
 * @package sae
 * @subpackage fetchurl
...
class FetchUrl {
    ...
    public function __construct($uniq_key, $need_proxy, $proxy_type = null, $proxy_addr = null, 
                         $proxy_need_auth = null, $proxy_auth_type = null,
                         $proxy_auth_usr = null, $proxy_auth_psw = null) {
        $this->uniq_key = $uniq_key;
        $this->need_proxy = $need_proxy;
        $this->proxy_addr = $proxy_addr;
        $this->proxy_type = strtoupper($proxy_type);
        $this->proxy_need_auth = $proxy_need_auth;
        $this->proxy_auth_type = strtoupper($proxy_auth_type);
        $this->proxy_auth_usr = $proxy_auth_usr;
        $this->proxy_auth_psw = $proxy_auth_psw;
        $this->errno_ = 0;
        $this->error_ = null;
        $this->debug_ = false;
    }
            ...
            if ($this->errno_ == 0) {
                if ($this->httpCode_ == 301 || $this->httpCode_ == 302) {
                    ...
                    if (preg_match('/Location:\s([^\r\n]+)/i', $this->header_, $matchs)) {
                        ...
                        if ($this->debug_) {
                            $this->debug_msg .= "[fetch debug] redirect to $url\n";
//                            echo "[fetch debug] redirect to $url\n";
                        }
                        continue;
                    }
                }
        ...
        if ($this->errno_ == 0 && $this->httpCode_ == 200) {
            ...
            if ($this->errno_  == CURLE_COULDNT_RESOLVE_HOST) {
                return false;
            }
            return null;
        }
    ...
    private function signature($url, $timestamp) {
        $content = "FetchUrl"  . $url .
            "TimeStamp" . $timestamp . $this->uniq_key;
        $signature = (base64_encode($content));
        if ($this->debug_) {
//            echo "[fetch debug] content: $content" . "\n";
//            echo "[fetch debug] signature: $signature" . "\n";
            $this->debug_msg .= "[fetch debug] content: $content" . "\n";
            $this->debug_msg .= "[fetch debug] signature: $signature" . "\n";
        }
        return $signature;
    }
...
//        if ($this->debug_) {
////            echo "[fetch debug] body => $body";
//            $this->debug_msg .= "[fetch debug] body => $body";
//        }
        return strlen($body);
    }
    ...
    public function writeHeader($ch, $header) {
        ...
        if ($this->debug_) {
//            echo $this->header_."\r\n";
//            echo "[fetch debug] header => $header";    
            $this->debug_msg .= "[fetch debug] header => $header";
        }
...
//        var_dump($this->need_proxy);
//        var_dump($this->proxy_type);
        if ($this->need_proxy) {
            switch ($this->proxy_type) {
                case "HTTP"   : curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_HTTP);
                break;
                case "SOCKS5" : curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
                break;
                case "SOCKS4" : curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS4);
            }
//            curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_HTTP);
            curl_setopt($ch, CURLOPT_PROXY, $this->proxy_addr);
//            var_dump($this->proxy_need_auth);
            if ($this->proxy_need_auth) {
                switch ($this->proxy_auth_type) {
                    case "BASIC" : curl_setopt($ch, CURLOPT_PROXYAUTH, CURLAUTH_BASIC);
//                    echo $this->proxy_auth_type."\n";break;
                    case "NTLM"  : curl_setopt($ch, CURLOPT_PROXYAUTH, CURLAUTH_NTLM);
//                	echo $this->proxy_auth_type."\n";break;
                }
//                var_dump($this->proxy_auth_type);
//                curl_setopt($ch, CURLOPT_PROXYAUTH, CURLAUTH_NTLM);
                curl_setopt($ch, CURLOPT_PROXYUSERPWD, "{$this->proxy_auth_usr}:{$this->proxy_auth_psw}");
            }
        }
...
//                    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, intval($v));
                    break;
                case 'sendtimeout':
                ...
                case 'readtimeout':
                    ...
                    break;
                case 'totaltimeout': 
                    curl_setopt($ch, CURLOPT_TIMEOUT, intval($v));
                    break;
                case 'followlocation':
                    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, (bool)$v);
//                    var_dump($v);
//                    exit();
                    break;
                default:
        ...
        if ($this->debug_) {
//            echo "[fetch debug] curl_getinfo => " . print_r($info, true) . "\n";
            $this->debug_msg .= "[fetch debug] curl_getinfo => " . print_r($info, true) . "\n";
        }
        ...
        if ($this->errno_ == 0) {
            ...
            if (preg_match('/^(?:[^\s]+)\s([^\s]+)\s([^\r\n]+)/', $this->header_, $matchs)) {
                ...
                if ($this->debug_) {
//                    echo "[fetch debug] httpCode = " . $this->httpCode_ . "  httpDesc = " . $this->httpDesc_ . "\n";
                    $this->debug_msg .= "[fetch debug] httpCode = " . $this->httpCode_ . "  httpDesc = " . $this->httpDesc_ . "\n";
                }
            } else {
                $this->errno_ = -1;
    ...
    private $proxy_addr;
    //代理是否需要用户验证
    private $proxy_need_auth;
    //代理验证类型：BASIC或NLMP
    private $proxy_auth_type;
    //代理验证的用户
    private $proxy_auth_usr;
    //代理验证的密码
    private $proxy_auth_psw;
}